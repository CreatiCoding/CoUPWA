 ...
 * 웹툰 메인 배너 리스트 가져오는 함수
 * num:  가져올 리스트의 갯수 ( 기본 6 )
 * getMainContentList().form({week:'mon'})
 * @type {HttpsFunction}
 */
exports.getMainContentList = functions.https.onRequest((req, res) => {
    const week = req.body.week === undefined ? (req.query.week === undefined ? 0 : req.query.week) : req.body.week;
    const referUrl = '"http://m.comic.naver.com/webtoon/weekday.nhn?week=' + week;
    const userAgent =
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' +
        ' (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36';
    request(referUrl, function(error, mres, body) {
        var $ = cheerio.load(body);
        //$("li div#AD_WEBTOON_MOBILEWEB_UP").parentNode.remove();
        var data = $('#pageList li');
        console.dir(data);
        res.send(true);
    });
});
...
exports.requestRefererWebtoon = functions.https.onRequest((req, res) => {
    const url = req.body.url === undefined ? (req.query.url === undefined ? 0 : req.query.url) : req.body.url;
    const referUrl = 'http://comic.naver.com/webtoon/weekday.nhn';
    const userAgent =
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' +
        ' (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36';
    const options = {
    ...
    request(options, function(mErr, mRes, mBody) {
        if (!mErr && mRes.statusCode == 200) {
            res.send('data:image/jpeg;base64,' + mBody);
        } else {
            console.error('[에러/다운로드]' + mErr.toString());
        }
    });
...
// http://thumb.comic.naver.net/webtoon/653344/127/thumbnail_202x120_11a144ad-32aa-4b96-8b62-c3e1e1c35b10.jpg
// http://localhost:5001/coupwa/us-central1/imagePipe?url=http://thumb.comic.naver.net/webtoon/653344/127/thumbnail_202x120_11a144ad-32aa-4b96-8b62-c3e1e1c35b10.jpg
exports.time = functions.https.onRequest((req, res) => {
	res.send(new Date(new Date().getTime() + 33 * 60 * 60 * 1000).toJSON());
